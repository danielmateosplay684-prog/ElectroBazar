...
public class DebugApiRestController {
    ...
    /**
     * Seeds the database with a massive amount of sales to test performance and
     * indexing.
     * Uses JdbcTemplate Batch updates for maximum performance, bypassing JPA
     * overhead.
     * 
    ...
    public String seedSales(@RequestParam(defaultValue = "30000") int count) {
        ...
        List<Long> productIds = jdbcTemplate.queryForList("SELECT id FROM products WHERE active = true LIMIT 100",
                Long.class);
        ...
        for (int i = 0; i < count; i += batchSize) {
            ...
            for (int j = 0; j < currentBatch; j++) {
                ...
                // In a REAL mass seeder, we'd use generated IDs, but this works for stress
                // testing.
        ...
        return String.format("Se han inyectado %d ventas y %d líneas correctamente en %.2f segundos.", count,
                totalLinesCount, seconds);
    }
}
