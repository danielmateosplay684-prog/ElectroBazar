...
public class TariffPriceHistoryServiceImpl implements TariffPriceHistoryService {
    ...
    public List<TariffPriceEntryDTO> getCurrentPricesForTariff(Long tariffId) {
        ...
        List<TariffPriceHistory> histories = tariffPriceHistoryRepository.findByTariffIdOrderByValidFromDesc(tariffId).stream()
                .filter(t -> t.getValidTo() == null)
                .collect(Collectors.toList());

        Map<Long, TariffPriceHistory> historyMap = histories.stream()
                .collect(Collectors.toMap(h -> h.getProduct().getId(), h -> h, (existing, replacement) -> existing));

        BigDecimal discountPercent = tariff.getDiscountPercentage() != null ? tariff.getDiscountPercentage() : BigDecimal.ZERO;
        ...
        for (Product product : products) {
            ...
            if (h != null) {
                ...
                BigDecimal vatRate = product.getTaxRate() != null && product.getTaxRate().getVatRate() != null ? product.getTaxRate().getVatRate() : new BigDecimal("0.21");
                BigDecimal reRate = recargoCalculator.getRecargoRate(vatRate);

                BigDecimal productNet = product.getPrice() != null ? product.getPrice().divide(BigDecimal.ONE.add(vatRate), 10, RoundingMode.HALF_UP) : BigDecimal.ZERO;
                BigDecimal netPrice = productNet.multiply(discountMultiplier).setScale(2, RoundingMode.HALF_UP);

                BigDecimal priceWithVat = netPrice.multiply(BigDecimal.ONE.add(vatRate)).setScale(2, RoundingMode.HALF_UP);
                BigDecimal priceWithRe = netPrice.multiply(BigDecimal.ONE.add(vatRate).add(reRate)).setScale(2, RoundingMode.HALF_UP);
